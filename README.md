...
- **Phasing**: `automate <track>.phase` offsets one track's notes from where they're written, in beats (`b + phase(b)`, negative = ahead). A linear ramp is a constant-rate drift, a hold is a lock — `0 R:96 -0.5:36 R:96` is one Reich phase step: one eighth ahead over six 6-beat figures, then locked. Only that track's notes move; automation stays on the song clock. `songs/tessera.song` is the demo.
...
- **Bounces**: `--play x.song --render out.wav` (add `--no-normalize` to keep the engine's exact gain; peak/RMS/LUFS are reported either way). `--render-stems <dir>` writes one un-normalized wav per track plus `_mix.wav` from a **single pass**: every voice renders once, and each track's strip feeds its own copy of the master chain (reverbs, chorus, tape) on worker threads, so a stem is the track exactly as it sits in the mix and the whole set costs little more than one bounce. It prints a per-stem level table — measured mixing without text-splitting song files. `--bits 16` writes TPDF-dithered 16-bit PCM instead of the default 32-bit float (half the size, plays anywhere). Every offline render also reports its peak concurrent voice count. `patina --help` lists every command.
